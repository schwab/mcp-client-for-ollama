...
from mcp_client_for_ollama.utils.base_tool_parser import BaseToolParser
...
def test_json_tool_parser_xml_style(json_tool_parser):
    text = """<tool_code>
<name>tool_xml</name>
<arguments>{"param": "value"}</arguments>
    ...
    tool_calls = json_tool_parser.parse(text)
    assert len(tool_calls) == 1
    assert tool_calls[0] == create_tool_call("tool_xml", {"param": "value"})
...
def test_composite_tool_parser_both_json_and_python(composite_tool_parser):
    text = """```json
...
```"""
    ...
    assert tool_calls[0].function.name == "valid_json"
    assert tool_calls[1].function.name == "builtin.execute_python_code"
